  ...
  return (
    <article className="bg-trama bg-paper pb-16 pt-28 lg:pb-24 lg:pt-32">
      ...
      <Container className={proyecto.heroVideo ? "mt-14" : "mt-6"}>
        ...
        <Reveal
          delay={0.1}
          className="relative mt-10 aspect-[16/10] w-full overflow-hidden rounded-2xl bg-stone-200 sm:aspect-[21/9]"
        >
          <Image
        ...
        {total > 1 && (
          <Reveal delay={0.2} className="mt-10 flex gap-3 overflow-x-auto pb-2">
            {imagenes.map((src, i) => (
              <button
                key={src + i}
                type="button"
                ...
                className={`relative h-20 w-28 shrink-0 overflow-hidden rounded-lg transition-opacity ${
